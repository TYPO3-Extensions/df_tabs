...
History.implement({
	...
	register: function(route) {
		if (this.routes.push(route) === 1) {
			this.addEvent('changed', this.match);
		}
	},
...
History.Route = new Class({
	...
	initialize: function(options) {
		...
		if (typeOf(this.pattern) === 'string') {
			this.pattern = new RegExp(this.pattern, this.options.flags);
		}
	...
	generate: function() {
		if (this.options.skipDefaultMatch && (String(this.values) === String(this.defaults))) {
			return null;
		}
	...
	match: function(state) {
		...
		if (bits) {
			...
			for (var i = 0, j = bits.length; i < j; ++i) {
				bits[i] = Array.pick([bits[i], defaults[i] || null]);
			}
			if (String(bits) !== String(defaults)) {
				this.values = bits;
			}
		} else {
